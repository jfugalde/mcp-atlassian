...
- Native Macross WhatsApp (`macross-whatsapp.js` + `macross-whatsapp.css`): desktop FAB, native 3-location picker, human Spanish pre-filled `wa.me` messages (no bot-like wording). Branch numbers editable in theme settings (Macross section). Guadalajara routes to CDMX WhatsApp for now. No Seedgrow / app embed. Routing: **hostname subdomains first** (`cdmx|puebla|guadalajara.farmaciasmacross.com.mx` → high confidence, no cookie); apex uses IP/GPS only after cookie accept; low-confidence geo shows picker; dev override `?macross_branch=cdmx|puebla|guadalajara`.
